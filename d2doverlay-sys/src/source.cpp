...
void draw_circle(float x, float y, float radius, float thickness, float r, float g, float b, float a, bool filled) {
	...
	if (filled)
		target->FillEllipse(D2D1::Ellipse(D2D1::Point2F(x, y), radius, radius), solid_brush);
...
void draw_ellipse(float x, float y, float width, float height, float thickness, float r, float g, float b, float a, bool filled) {
	...
	if (filled)
		target->FillEllipse(D2D1::Ellipse(D2D1::Point2F(x, y), width, height), solid_brush);

	else target->DrawEllipse(D2D1::Ellipse(D2D1::Point2F(x, y), width, height), solid_brush, thickness);
}

void d2oSetup(const HWND& tWindow) {
...
DWORD PidFromProcessName(const std::string &process) {
	PROCESSENTRY32W pe;
	...
	do {
		if (!lstrcmpiW(pe.szExeFile, std::wstring(process.begin(), process.end()).c_str())) {
			CloseHandle(handle);
